...
parser.add_argument("poses_dir", nargs='?', default="", help="Path to where the poses are saved")

args = parser.parse_args()

print('poses_dir',args.poses_dir)
...
output_path = "scene.ply"  # Specify the output file path
bpy.ops.export_mesh.ply(filepath=output_path)
"""
def check_name(name):
    print('name', name)
    for category_name in ["chair", "sofa", "table", "bed", "lamp", "cabinet", 'floor', 'others', 'ceiling', 'door', 'window', 'wallbottom', 'hole']:
...
directory_path = os.path.dirname(pose_init)
master_bedroom_path = os.path.dirname(directory_path)

poses_tmp_dir = os.path.join(master_bedroom_path, 'poses_tmp')
os.makedirs(poses_tmp_dir, exist_ok=True)
...
os.rename(poses_tmp_dir, directory_path)
print('Renamed directory', poses_tmp_dir, 'to: ', directory_path)
